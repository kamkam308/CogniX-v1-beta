...
function loadSessionSecret(key: string, fallback: string): string {
  if (!canUseStorage()) return fallback;
  try {
...
function saveSessionSecret(key: string, value: string): void {
  if (!canUseStorage()) return;
  try {
    localStorage.removeItem(key);
...
export const useChatRuntimeStore = create<ChatRuntimeStore>((set, get) => ({
  ...
  cancelByThreadId: {},
  autoTitle: false,
  hfToken: loadSessionSecret(HF_TOKEN_KEY, ""),
  modelsError: null,
  ...
  setHfToken: (hfToken) => {
    saveSessionSecret(HF_TOKEN_KEY, hfToken);
    set({ hfToken });
