...
function externalConflictMessage(preflight: DesktopPreflightResult) {
  if (preflight.reason === "desktop_owned_backend_active") {
    return preflight.port
      ? `A desktop-owned CogniX server for this install is already running on port ${preflight.port}. Quit the other desktop app instance, then try again.`
      : "A desktop-owned CogniX server for this install is already running. Quit the other desktop app instance, then try again.";
  }

  if (preflight.reason === "desktop_owned_backend_starting") {
    return "The desktop-owned CogniX backend is still starting. Wait a moment, then try again.";
  }

  if (preflight.reason?.startsWith("desktop_owned_backend_unmanageable:")) {
    return preflight.port
      ? `A desktop-owned CogniX backend on port ${preflight.port} cannot be safely controlled by this desktop app. Stop that backend, then reopen CogniX.`
      : "A desktop-owned CogniX backend cannot be safely controlled by this desktop app. Stop that backend, then reopen CogniX.";
  }

  return preflight.port
    ? `A CogniX server for this install is already running from a terminal on port ${preflight.port}. Stop that server, or run \`unsloth studio update\` from that terminal before using the desktop app.`
    : "A CogniX server for this install is already running from a terminal. Stop that server, or run `unsloth studio update` from that terminal before using the desktop app.";
}
          ...
          if (preflight.can_auto_repair) {
            ...
            setBackendError(
              preflight.disposition === "owned_stale"
                ? "Desktop-owned CogniX backend is too old for this desktop app. Run `unsloth studio update`, then restart CogniX."
                : "Managed CogniX install is too old. Run `unsloth studio update`.",
            );
          }
          return;
  ...
  async function startManagedServer() {
    ...
    try {
      ...
      if (msg.includes("already running")) {
        startingRef.current = false;
        setBackendError(
          "Managed server is already running but did not report a port. Restart CogniX and try again.",
        );
        return;
      }
      setBackendError(msg);
