...
"""
 Main FastAPI application for CogniX Backend
"""
...
def _require_admin_subject(current_subject: str) -> None:
...
app = FastAPI(
    title = "CogniX Backend",
    version = UNSLOTH_VERSION,
    description = "Backend API for CogniX",
    lifespan = lifespan,
)
...
_AUTH_REQUEST_BODY_MAX_BYTES = 64 * 1024
_BODY_OPENAI_COMPAT_PREFIXES = (
    "/v1/chat/completions",
    "/v1/completions",
    "/v1/responses",
    "/v1/embeddings",
...
_COGNIX_SECURITY_SCAN_BODY_PREFIXES = (
    "/api/auth",
    "/api/cognix",
    "/api/settings",
    "/api/providers",
...
def _get_protected_request_max_bytes(path: str | None = None) -> int:
    if path and path.startswith("/api/auth"):
        return _AUTH_REQUEST_BODY_MAX_BYTES
    return default_request_body_limit_bytes()


_SUSPICIOUS_REQUEST_PATTERNS = (
    _re.compile(r"(?i)(?:union\s+select|select\s+.+\s+from\s+|insert\s+into|drop\s+table|information_schema)"),
    _re.compile(r"(?i)(?:\bor\b\s+1\s*=\s*1|\band\b\s+1\s*=\s*1|--\s*$|/\*|\*/)"),
    _re.compile(r"(?i)(?:<\s*script\b|javascript:|onerror\s*=|onload\s*=|data:text/html)"),
    _re.compile(r"(?i)(?:\.\./|\.\.\\|%2e%2e|%252e%252e)"),
    _re.compile(r"(?i)(?:;\s*(?:rm\s+-rf|shutdown|powershell|cmd\.exe|curl\s+|wget\s+))"),
)


async def _send_security_rejection(send) -> None:
    payload = _json_for_413.dumps({"detail": "Request rejected."}).encode("utf-8")
    await send(
        {
...
class SuspiciousRequestMiddleware:
    """Standalone scanner used by isolated middleware tests.

    Runtime V1 uses CognixSecurityScanMiddleware, which also records security
    events and applies admin-review bans.
    """

    def __init__(self, app, scan_prefixes: tuple = _COGNIX_SECURITY_SCAN_BODY_PREFIXES):
        self.app = app
    ...
    async def __call__(self, scope, receive, send):
        if scope["type"] != "http":
            ...
            return
        path = scope.get("path", "")
        if not any(path.startswith(prefix) for prefix in self.scan_prefixes):
            await self.app(scope, receive, send)
            return
        sample = _cognix_security_sample_from_scope(scope)
        if any(pattern.search(sample) for pattern in _SUSPICIOUS_REQUEST_PATTERNS):
            await _send_security_rejection(send)
            return
        await self.app(scope, receive, send)


_CROSS_SITE_WRITE_METHODS = ("POST", "PUT", "PATCH", "DELETE")
_CROSS_SITE_WRITE_PROTECTED_PREFIXES = (
    "/api/",
    "/v1/",
)


async def _send_cross_site_rejection(send) -> None:
    payload = _json_for_413.dumps({"detail": "Cross-site request rejected."}).encode("utf-8")
    await send(
        {
            "type": "http.response.start",
            "status": 403,
...
def _headers_from_scope(scope) -> dict[str, str]:
    ...
    for name, value in scope.get("headers", []):
        try:
            ...
            continue
        headers.setdefault(key, text)
    return headers


def _origin_tuple_from_url(value: str) -> tuple[str, str, int] | None:
    try:
...
def _origin_tuple_from_scope(scope, headers: dict[str, str]) -> tuple[str, str, int] | None:
    ...
    if host_header:
        origin = _origin_tuple_from_url(f"{scheme}://{host_header}")
...
def _same_origin_header_value(scope, headers: dict[str, str], value: str) -> bool:
    if not value or value == "null":
        return False
    request_origin = _origin_tuple_from_scope(scope, headers)
    header_origin = _origin_tuple_from_url(value)
    return request_origin is not None and header_origin == request_origin


class CrossSiteWriteMiddleware:
    """Reject browser-originated cross-site writes before auth/cookies are used.

    Missing browser provenance headers are allowed so local CLI/API clients remain
    compatible; browsers send Origin and/or Fetch Metadata on cross-site writes.
    """

    def __init__(
        self,
        app,
        protected_prefixes: tuple = _CROSS_SITE_WRITE_PROTECTED_PREFIXES,
        trusted_origins: tuple | None = None,
    ):
        self.app = app
        self.protected_prefixes = protected_prefixes
        configured = trusted_origins
        if configured is None:
            configured = tuple(globals().get("_cors_origins", ()))
        self.trusted_origin_strings = {
            origin.strip().rstrip("/") for origin in configured if isinstance(origin, str) and origin.strip()
        }
        self.trusted_origin_tuples = {
            parsed for origin in self.trusted_origin_strings if (parsed := _origin_tuple_from_url(origin))
        }

    def _is_trusted_origin(self, value: str) -> bool:
        normalized = (value or "").strip().rstrip("/")
        if normalized in self.trusted_origin_strings:
            return True
        parsed = _origin_tuple_from_url(normalized)
        return parsed is not None and parsed in self.trusted_origin_tuples
    ...
    async def __call__(self, scope, receive, send):
        ...
        path = scope.get("path", "")
        if method not in _CROSS_SITE_WRITE_METHODS or not any(
        ...
        if sec_fetch_site == "cross-site":
            await _send_cross_site_rejection(send)
            return

        origin = headers.get("origin")
        if (
            origin is not None
            and not _same_origin_header_value(scope, headers, origin)
            and not self._is_trusted_origin(origin)
        ):
            await _send_cross_site_rejection(send)
            return
        ...
        if (
            origin is None
            and referer
            and not _same_origin_header_value(scope, headers, referer)
            and not self._is_trusted_origin(referer)
        ):
            await _send_cross_site_rejection(send)
            return
...
async def _send_403_banned(send, detail: str) -> None:
    payload = _json_for_413.dumps({"detail": detail}).encode("utf-8")
    await send(
        {
            "type": "http.response.start",
            "status": 403,
...
def _cognix_subject_from_scope(scope) -> str | None:
    ...
    try:
        ...
        return _decode_subject_without_verification(token)
    except Exception:
        return None


def _cognix_bearer_token_from_scope(scope) -> str | None:
    authorization = _header_value(scope, b"authorization") or ""
    if not authorization.lower().startswith("bearer "):
        return None
    token = authorization.split(" ", 1)[1].strip()
...
async def _cognix_verified_jwt_subject_from_scope(scope) -> str | None:
    token = _cognix_bearer_token_from_scope(scope)
    if not token or token.startswith("sk-"):
        return None
    try:
        from fastapi.security import HTTPAuthorizationCredentials
        from auth.authentication import get_current_jwt_subject_allow_password_change

        return await get_current_jwt_subject_allow_password_change(
...
def _cognix_client_key(scope) -> str | None:
...
def _cognix_security_sample_from_scope(scope, body_text: str | None = None) -> str:
    path = scope.get("path", "")
    parts = [path]
    ...
    if query_string:
        try:
            ...
            query_text = ""
        if query_text:
            parts.append(f"{path}?{query_text}")
            decoded_query = unquote_plus(query_text)
            if decoded_query and decoded_query != query_text:
                parts.append(f"{path}?{decoded_query}")
    if body_text:
        parts.append(body_text)
    return "\n".join(parts)


class CognixBanMiddleware:
    """Block API reads/writes for JWT sessions under active admin-review bans."""
    ...
    async def __call__(self, scope, receive, send):
        ...
        username = await _cognix_verified_jwt_subject_from_scope(scope)
        client_key = _cognix_client_key(scope)
        if not username and not client_key:
            await self.app(scope, receive, send)
            return
        try:
            from auth import storage as auth_storage
            from storage.cognix_db import active_ban_for

            if username and auth_storage.is_admin(username):
                await self.app(scope, receive, send)
                return
            if not username and path.startswith("/api/auth/"):
                active_ban = None
            else:
                active_ban = active_ban_for(username, client_key)
        except Exception:
            active_ban = None
        if active_ban:
            await _send_403_banned(
                send,
                "Account temporarily locked. Contact your administrator to unlock your account.",
            )
            return
...
class CognixSecurityScanMiddleware:
    """Detect known attack signatures in API paths and query strings."""

    def __init__(self, app, scan_prefixes: tuple):
        self.app = app
        self.scan_prefixes = scan_prefixes
    ...
    async def __call__(self, scope, receive, send):
        ...
        security_sample = _cognix_security_sample_from_scope(scope)

        try:
            from auth import storage as auth_storage
            from storage.cognix_db import detect_known_attack, record_security_event

            username = await _cognix_verified_jwt_subject_from_scope(scope)
            if username and auth_storage.is_admin(username):
                await self.app(scope, receive, send)
                return
            attack = detect_known_attack(security_sample)
            if attack:
                record_security_event(
                    ...
                    pattern_label = attack["pattern_label"],
                    method = scope.get("method"),
                    path = path,
                    excerpt = security_sample[:1000],
                    create_temporary_ban = True,
                )
                await _send_403_banned(
                    send,
                    "Security threat detected. Your access is temporarily locked until administrator review.",
                )
                return
        except Exception:
            pass
...
class MaxBodyMiddleware:
    ...
    def _max_bytes(self, path: str) -> int:
        try:
            return int(self.max_bytes_getter(path))
        except TypeError:
    ...
    async def __call__(self, scope, receive, send):
        ...
        username = await _cognix_verified_jwt_subject_from_scope(scope)
        client_key = _cognix_client_key(scope)
        is_admin_subject = False
        try:
            from auth import storage as auth_storage
            from storage.cognix_db import active_ban_for

            is_admin_subject = bool(username and auth_storage.is_admin(username))
            active_ban = None if is_admin_subject else active_ban_for(username, client_key)
        except Exception:
            active_ban = None
        if active_ban:
            await _send_403_banned(
        ...
        max_bytes = self._max_bytes(path)
        declared = None
        for name, value in scope.get("headers", []):
        ...
        body_text = None
        if any(path.startswith(p) for p in _COGNIX_SECURITY_SCAN_BODY_PREFIXES) and chunks:
            body_text = b"".join(chunks).decode("utf-8", errors = "ignore")
        security_sample = _cognix_security_sample_from_scope(scope, body_text = body_text)
        try:
            from storage.cognix_db import detect_known_attack, record_security_event

            attack = None if is_admin_subject else detect_known_attack(security_sample)
            if attack:
                record_security_event(
                    username = username,
...
app.add_middleware(CognixBanMiddleware)

app.add_middleware(
    CognixSecurityScanMiddleware,
    scan_prefixes = _BODY_PROTECTED_PREFIXES,
)

app.add_middleware(
    MaxBodyMiddleware,
    max_bytes_getter = _get_protected_request_max_bytes,
...
app.add_middleware(CrossSiteWriteMiddleware)
...
def _configured_cors_origins(default_origins: list[str]) -> list[str]:
    """Return explicit CORS origins, never wildcard with credentials.

    Same-origin browser traffic does not need CORS. Extra cross-origin frontends
    can opt in with UNSLOTH_STUDIO_CORS_ORIGINS as a comma-separated list.
    """
    origins = list(default_origins)
    for raw in os.environ.get("UNSLOTH_STUDIO_CORS_ORIGINS", "").split(","):
        origin = raw.strip()
...
app.include_router(cognix_router, prefix = "/api/cognix", tags = ["cognix"])
...
async def health_check(request: Request):
    ...
    if not auth.lower().startswith("bearer "):
        return base
    try:
        from auth.authentication import get_current_jwt_subject as _gcs
        from fastapi.security import HTTPAuthorizationCredentials
...
def studio_install_source(_current_subject: str = Depends(get_current_jwt_subject)):
...
def studio_update_status(_current_subject: str = Depends(get_current_jwt_subject)):
...
def studio_download_transport_capabilities(_current_subject: str = Depends(get_current_jwt_subject)):
...
async def shutdown_server(request: Request, current_subject: str = Depends(get_current_jwt_subject)):
    ...
    """
    _require_admin_subject(current_subject)
...
@app.get("/api/system")
async def get_system_info(current_subject: str = Depends(get_current_jwt_subject)):
...
async def get_gpu_visibility(current_subject: str = Depends(get_current_jwt_subject)):
    return get_backend_visible_gpu_info()


@app.get("/api/system/hardware")
def get_hardware_info(
    include_details: bool = Query(False), current_subject: str = Depends(get_current_jwt_subject)
):
