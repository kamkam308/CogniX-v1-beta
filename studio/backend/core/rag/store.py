...
DEFAULT_KB_OWNER_USERNAME = "unsloth"
...
def create_kb(
    ...
    *,
    ...
    kb_id: str | None = None,
    owner_username: str = DEFAULT_KB_OWNER_USERNAME,
) -> str:
    kb_id = kb_id or str(uuid.uuid4())
    conn.execute(
        "INSERT INTO knowledge_bases(id, name, description, embedding_model, created_at, owner_username) "
        "VALUES(?,?,?,?,?,?)",
        (kb_id, name, description, embedding_model, _now(), owner_username or DEFAULT_KB_OWNER_USERNAME),
    )
    conn.commit()
    return kb_id
...
def list_kbs(
    ...
    *,
    ...
    if owner_username is not None and not include_all:
        rows = conn.execute(
            "SELECT * FROM knowledge_bases WHERE owner_username=? ORDER BY created_at",
            (owner_username,),
        ).fetchall()
    else:
...
def get_kb(
    ...
    *,
    ...
    if owner_username is not None and not include_all:
        row = conn.execute(
            "SELECT * FROM knowledge_bases WHERE id=? AND owner_username=?",
            (kb_id, owner_username),
        ).fetchone()
    else:
