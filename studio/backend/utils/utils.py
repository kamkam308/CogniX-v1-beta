...
_API_KEY_RE = re.compile(r"\b(?:sk|sk-proj|sk-ant|sk-or)-[A-Za-z0-9._-]{8,}\b")
_SENSITIVE_ASSIGNMENT_RE = re.compile(
    r"(?i)\b(api[_-]?key|token|authorization|password|secret)\s*[:=]\s*[^,\s;]+"
)
...
def safe_curated_detail(error: Exception, fallback: str = "An internal error occurred") -> str:
    ...
    msg = _BEARER_TOKEN_RE.sub(r"\1 <redacted token>", msg)
    msg = _HF_TOKEN_RE.sub("<redacted hf token>", msg)
    msg = _API_KEY_RE.sub("<redacted api key>", msg)
    msg = _SENSITIVE_ASSIGNMENT_RE.sub(r"\1=<redacted>", msg)
