...
from hub.dependencies import get_hf_token, require_admin_subject
...
router = APIRouter(dependencies = [Depends(require_admin_subject)])
...
async def list_local_models(
    ...
    current_subject: str = Depends(require_admin_subject),
):
...
def get_scan_folders(current_subject: str = Depends(require_admin_subject)):
...
def add_scan_folder_endpoint(
    body: AddScanFolderRequest, current_subject: str = Depends(require_admin_subject)
):
...
def remove_scan_folder_endpoint(
    folder_id: int, current_subject: str = Depends(require_admin_subject)
):
...
def get_recommended_folders(current_subject: str = Depends(require_admin_subject)):
...
def get_models_folder(current_subject: str = Depends(require_admin_subject)):
...
async def cancel_download_model(
    body: CancelDownloadRequest, current_subject: str = Depends(require_admin_subject)
):
