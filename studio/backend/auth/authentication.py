...
def refresh_access_token(refresh_token: str) -> Tuple[Optional[str], Optional[str], bool]:
    ...
    """
    consumed = consume_refresh_token(refresh_token)
    if consumed is None:
...
async def get_current_jwt_subject(
    credentials: HTTPAuthorizationCredentials = Depends(security),
) -> str:
    """Validate a browser/desktop JWT and reject API keys for UI/admin routes."""
...
async def get_current_jwt_subject_allow_password_change(
    credentials: HTTPAuthorizationCredentials = Depends(security),
) -> str:
    """Validate a browser/desktop JWT for password-change flow; reject API keys."""
    if credentials.credentials.startswith(API_KEY_PREFIX):
        raise HTTPException(
            status_code = status.HTTP_403_FORBIDDEN,
            detail = "Web session required",
