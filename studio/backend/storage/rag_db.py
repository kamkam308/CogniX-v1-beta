...
def _ensure_schema(conn: sqlite3.Connection) -> None:
    ...
    kb_cols = {r[1] for r in conn.execute("PRAGMA table_info(knowledge_bases)").fetchall()}
    if "owner_username" not in kb_cols:
        conn.execute("ALTER TABLE knowledge_bases ADD COLUMN owner_username TEXT NOT NULL DEFAULT 'unsloth'")
    conn.execute(
        "UPDATE knowledge_bases SET owner_username = ? WHERE owner_username IS NULL OR owner_username = ''",
        ("unsloth",),
    )
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_kb_owner_created_at ON knowledge_bases(owner_username, created_at)"
    )
