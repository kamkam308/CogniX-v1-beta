...
DEFAULT_CHAT_OWNER_USERNAME = "unsloth"
...
def _has_composite_owner_pk(conn: sqlite3.Connection, table: str, key_column: str) -> bool:
    columns = _table_columns(conn, table)
    pk_cols = [row["name"] for row in sorted((row for row in columns if row["pk"]), key = lambda r: r["pk"])]
    return pk_cols == ["owner_username", key_column]


def _migrate_owned_key_value_table(conn: sqlite3.Connection, table: str, key_column: str) -> None:
    if _has_composite_owner_pk(conn, table, key_column):
        return
    legacy = f"{table}_legacy_owner_migration"
    conn.execute(f"ALTER TABLE {table} RENAME TO {legacy}")
    conn.execute(
        f"""
        CREATE TABLE {table} (
            owner_username TEXT NOT NULL DEFAULT '{DEFAULT_CHAT_OWNER_USERNAME}',
            {key_column} TEXT NOT NULL,
            value_json TEXT NOT NULL,
            updated_at TEXT NOT NULL,
            PRIMARY KEY(owner_username, {key_column})
        )
        """
    )
    conn.execute(
        f"""
        INSERT OR REPLACE INTO {table} (owner_username, {key_column}, value_json, updated_at)
        SELECT ?, {key_column}, value_json, updated_at FROM {legacy}
        """,
...
def _migrate_owned_prompt_table(conn: sqlite3.Connection, table: str, content_column: str) -> None:
    if _has_composite_owner_pk(conn, table, "id"):
        return
    legacy = f"{table}_legacy_owner_migration"
    conn.execute(f"ALTER TABLE {table} RENAME TO {legacy}")
    conn.execute(
    ...
    conn.execute(
        ...
        INSERT OR REPLACE INTO {table}
            (owner_username, id, name, {content_column}, created_at, updated_at)
        SELECT ?, id, name, {content_column}, created_at, updated_at FROM {legacy}
        """,
        (DEFAULT_CHAT_OWNER_USERNAME,),
    )
    conn.execute(f"DROP TABLE {legacy}")
...
def _ensure_owned_user_tables(conn: sqlite3.Connection) -> None:
    _migrate_owned_key_value_table(conn, "chat_settings", "key")

    quarantine_cols = {row["name"] for row in _table_columns(conn, "chat_settings_quarantine")}
    if "owner_username" not in quarantine_cols:
        conn.execute(
            f"ALTER TABLE chat_settings_quarantine ADD COLUMN owner_username TEXT NOT NULL DEFAULT '{DEFAULT_CHAT_OWNER_USERNAME}'"
        )

    _migrate_owned_prompt_table(conn, "prompt_entries", "text")
    _migrate_owned_prompt_table(conn, "prompt_lists", "items_json")

    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_chat_settings_owner_key ON chat_settings(owner_username, key)"
    )
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_prompt_entries_owner_created_at ON prompt_entries(owner_username, created_at)"
    )
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_prompt_lists_owner_created_at ON prompt_lists(owner_username, created_at)"
    )
...
def _ensure_schema(conn: sqlite3.Connection) -> None:
    ...
    if "owner_username" not in chat_project_cols:
        conn.execute(
            f"ALTER TABLE chat_projects ADD COLUMN owner_username TEXT NOT NULL DEFAULT '{DEFAULT_CHAT_OWNER_USERNAME}'"
        )
    conn.execute(
        "UPDATE chat_projects SET owner_username = ? WHERE owner_username IS NULL OR owner_username = ''",
        (DEFAULT_CHAT_OWNER_USERNAME,),
    )
    conn.execute(
    ...
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_chat_projects_owner_archived_updated_at ON chat_projects(owner_username, archived, updated_at)"
    )
    conn.execute(
        """
    ...
    if "owner_username" not in chat_thread_cols:
        conn.execute(
            f"ALTER TABLE chat_threads ADD COLUMN owner_username TEXT NOT NULL DEFAULT '{DEFAULT_CHAT_OWNER_USERNAME}'"
        )
    conn.execute(
        "UPDATE chat_threads SET owner_username = ? WHERE owner_username IS NULL OR owner_username = ''",
        (DEFAULT_CHAT_OWNER_USERNAME,),
    )
    conn.execute(
        """
    ...
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_chat_threads_owner_created_at ON chat_threads(owner_username, created_at)"
    )
    conn.execute(
    ...
    conn.execute(
        ...
        CREATE TABLE IF NOT EXISTS chat_settings (
            owner_username TEXT NOT NULL DEFAULT 'unsloth',
            key TEXT NOT NULL,
            value_json TEXT NOT NULL,
            updated_at TEXT NOT NULL,
    ...
    _ensure_owned_user_tables(conn)
...
def list_prompt_entries(owner_username: str = DEFAULT_CHAT_OWNER_USERNAME) -> list[dict]:
    conn = get_connection()
    try:
        rows = conn.execute(
            "SELECT * FROM prompt_entries WHERE owner_username = ? ORDER BY created_at DESC",
            (owner_username,),
        ).fetchall()
...
def upsert_prompt_entry(
    entry: dict, owner_username: str = DEFAULT_CHAT_OWNER_USERNAME
) -> dict:
    conn = get_connection()
    try:
        conn.execute(
            """
            INSERT INTO prompt_entries (owner_username, id, name, text, created_at, updated_at)
            VALUES (?, ?, ?, ?, ?, ?)
...
def delete_prompt_entry(
    entry_id: str, owner_username: str = DEFAULT_CHAT_OWNER_USERNAME
) -> None:
    conn = get_connection()
    try:
        conn.execute(
            "DELETE FROM prompt_entries WHERE owner_username = ? AND id = ?",
            (owner_username, entry_id),
        )
        conn.commit()
    finally:
        conn.close()
...
def bulk_upsert_prompt_entries(
    entries: list[dict], owner_username: str = DEFAULT_CHAT_OWNER_USERNAME
) -> int:
    if not entries:
        return 0
    conn = get_connection()
    try:
        conn.executemany(
            ...
            """,
            [
                (owner_username, e["id"], e["name"], e["text"], e["createdAt"], e["updatedAt"])
                for e in entries
            ],
        )
        conn.commit()
...
def list_prompt_lists_db(owner_username: str = DEFAULT_CHAT_OWNER_USERNAME) -> list[dict]:
    conn = get_connection()
    try:
        rows = conn.execute(
            "SELECT * FROM prompt_lists WHERE owner_username = ? ORDER BY created_at DESC",
            (owner_username,),
        ).fetchall()
...
def upsert_prompt_list(
    lst: dict, owner_username: str = DEFAULT_CHAT_OWNER_USERNAME
) -> dict:
    conn = get_connection()
    try:
        conn.execute(
            """
            INSERT INTO prompt_lists (owner_username, id, name, items_json, created_at, updated_at)
            VALUES (?, ?, ?, ?, ?, ?)
            ON CONFLICT(owner_username, id) DO UPDATE SET
                name = excluded.name,
...
def delete_prompt_list_db(
    list_id: str, owner_username: str = DEFAULT_CHAT_OWNER_USERNAME
) -> None:
    conn = get_connection()
    try:
        conn.execute(
            "DELETE FROM prompt_lists WHERE owner_username = ? AND id = ?",
            (owner_username, list_id),
        )
        conn.commit()
    finally:
        conn.close()
...
def bulk_upsert_prompt_lists(
    lists: list[dict], owner_username: str = DEFAULT_CHAT_OWNER_USERNAME
) -> int:
    if not lists:
        return 0
    conn = get_connection()
...
def _chat_thread_from_row(row: sqlite3.Row) -> dict:
    ...
    return {
        ...
        "ownerUsername": data.get("owner_username") or DEFAULT_CHAT_OWNER_USERNAME,
    }
...
def upsert_chat_thread(
    thread: dict,
    *,
    owner_username: str = DEFAULT_CHAT_OWNER_USERNAME,
) -> dict:
    conn = get_connection()
    try:
        conn.execute(
            ...
            INSERT INTO chat_threads
                (id, title, model_type, model_id, pair_id, project_id, archived, created_at, openai_code_exec_container_id, anthropic_code_exec_container_id, forked_from_thread_id, forked_from_message_id, owner_username)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ON CONFLICT(id) DO UPDATE SET
                ...
                owner_username = COALESCE(chat_threads.owner_username, excluded.owner_username)
            """,
            (
                thread["id"],
    ...
    *,
    ...
    for key, (column, value) in allowed.items():
        if key in patch:
            ...
            values.append(value)
    if not assignments:
        return get_chat_thread(id, owner_username = owner_username, include_all = include_all)

    conn = get_connection()
    try:
        where = "id = ?"
        final_values = [*values, id]
        if owner_username is not None and not include_all:
            where += " AND owner_username = ?"
            final_values.append(owner_username)
        conn.execute(
            f"UPDATE chat_threads SET {', '.join(assignments)} WHERE {where}",
            final_values,
        )
        conn.commit()
        if owner_username is not None and not include_all:
            row = conn.execute(
                "SELECT * FROM chat_threads WHERE id = ? AND owner_username = ?",
                (id, owner_username),
            ).fetchone()
        else:
...
def delete_chat_threads(
    ...
    *,
    ...
    try:
        if owner_username is not None and not include_all:
            conn.executemany(
                "DELETE FROM chat_threads WHERE id = ? AND owner_username = ?",
                [(id, owner_username) for id in ids],
            )
        else:
...
def clear_chat_history(
    *,
    ...
    try:
        if owner_username is not None and not include_all:
            conn.execute("DELETE FROM chat_threads WHERE owner_username = ?", (owner_username,))
        else:
...
def count_chat_threads(
    *,
    ...
    try:
        if owner_username is not None and not include_all:
            row = conn.execute(
                "SELECT COUNT(*) FROM chat_threads WHERE owner_username = ?",
                (owner_username,),
            ).fetchone()
        else:
            row = conn.execute("SELECT COUNT(*) FROM chat_threads").fetchone()
        return int(row[0])
    finally:
        conn.close()
    ...
    try:
        conn.execute(
            ...
            INSERT INTO chat_projects
                (id, name, instructions, root_path, archived, created_at, updated_at, owner_username)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            ON CONFLICT(id) DO UPDATE SET
                name = excluded.name,
                ...
                created_at = excluded.created_at,
                updated_at = excluded.updated_at,
                owner_username = COALESCE(chat_projects.owner_username, excluded.owner_username)
            """,
            (
                project["id"],
                project["name"],
    ...
    *,
    ...
    for key, (column, value) in allowed.items():
        if key in patch:
            ...
            values.append(value)
    if not assignments:
        return get_chat_project(id, owner_username = owner_username, include_all = include_all)
    ...
    try:
        ...
        if owner_username is not None and not include_all:
            row = conn.execute(
                "SELECT * FROM chat_projects WHERE id = ? AND owner_username = ?",
                (id, owner_username),
            ).fetchone()
        else:
...
def ensure_chat_project_workspace(
    id: str,
    *,
    owner_username: str | None = None,
    include_all: bool = True,
) -> Optional[dict]:
    project = get_chat_project(id, owner_username = owner_username, include_all = include_all)
    if project is None:
        return None
...
def delete_chat_project(
    ...
    *,
    ...
    try:
        ...
        if owner_username is not None and not include_all:
            conn.execute(
                "DELETE FROM chat_threads WHERE project_id = ? AND owner_username = ?",
                (id, owner_username),
            )
            conn.execute(
                "DELETE FROM chat_projects WHERE id = ? AND owner_username = ?",
                (id, owner_username),
            )
        else:
...
def _load_chat_settings_for_merge(
    conn: sqlite3.Connection, owner_username: str
) -> tuple[dict[str, Any], set[str]]:
    rows = conn.execute(
        "SELECT key, value_json FROM chat_settings WHERE owner_username = ?",
        (owner_username,),
    ).fetchall()
    ...
    for row in rows:
        ...
        conn.execute(
            ...
            INSERT INTO chat_settings_quarantine
                (owner_username, key, value_json, reason, quarantined_at)
            VALUES (?, ?, ?, ?, ?)
            """,
            (owner_username, row["key"], row["value_json"], "json_decode_error", now),
        )
        conn.execute(
            "DELETE FROM chat_settings WHERE owner_username = ? AND key = ? AND value_json = ?",
            (owner_username, row["key"], row["value_json"]),
        )
        ...
        conn.execute(
            ...
            INSERT INTO chat_threads
                (id, title, model_type, model_id, pair_id, project_id, archived, created_at,
                 ...
                 forked_from_thread_id, forked_from_message_id, owner_username)
            VALUES (?, ?, ?, ?, ?, ?, 0, ?, NULL, NULL, ?, ?, ?)
            """,
            (
                new_thread_id,
...
def list_chat_settings(owner_username: str = DEFAULT_CHAT_OWNER_USERNAME) -> dict[str, Any]:
    conn = get_connection()
    try:
        rows = conn.execute(
            "SELECT key, value_json FROM chat_settings WHERE owner_username = ? ORDER BY key",
            (owner_username,),
        ).fetchall()
...
def upsert_chat_settings(
    ...
    if not settings:
        return list_chat_settings(owner_username = owner_username)
    conn = get_connection()
    try:
        now = datetime.now(timezone.utc).isoformat()
        conn.executemany(
            """
            INSERT INTO chat_settings (owner_username, key, value_json, updated_at)
            VALUES (?, ?, ?, ?)
            ON CONFLICT(owner_username, key) DO UPDATE SET
                value_json = excluded.value_json,
                updated_at = excluded.updated_at
            """,
            [(owner_username, key, json.dumps(value), now) for key, value in settings.items()],
        )
        conn.commit()
        return list_chat_settings(owner_username = owner_username)
    finally:
...
def upsert_chat_settings_merge(
    ...
    try:
        conn.execute("BEGIN IMMEDIATE")
        current, corrupt = _load_chat_settings_for_merge(conn, owner_username)
        unsafe_partial_keys = [
        ...
        conn.executemany(
            ...
            ON CONFLICT(owner_username, key) DO UPDATE SET
                ...
                updated_at = excluded.updated_at
            """,
            [(owner_username, key, json.dumps(value), now) for key, value in merged.items()],
        )
        conn.commit()
        return merged
