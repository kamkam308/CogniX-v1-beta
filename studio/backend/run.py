...
def run_server(
    ...
    Args:
        ...
        ssl_certfile/ssl_keyfile: optional local TLS certificate pair for native
            HTTPS serving (also configurable with COGNIX_SSL_CERTFILE /
            COGNIX_SSL_KEYFILE)
    ...
    ssl_certfile = ssl_certfile or os.environ.get("COGNIX_SSL_CERTFILE")
    ssl_keyfile = ssl_keyfile or os.environ.get("COGNIX_SSL_KEYFILE")
    local_https = bool(ssl_certfile or ssl_keyfile)
    if local_https:
        if not ssl_certfile or not ssl_keyfile:
            raise SystemExit(
                "Local HTTPS requires both COGNIX_SSL_CERTFILE and COGNIX_SSL_KEYFILE."
            )
        cert_path = Path(ssl_certfile).expanduser()
        key_path = Path(ssl_keyfile).expanduser()
        if not cert_path.is_file():
            raise SystemExit(f"Local HTTPS certificate not found: {cert_path}")
        if not key_path.is_file():
            raise SystemExit(f"Local HTTPS private key not found: {key_path}")
    ...
    if port and port > 0:
        ...
        app.state.server_url = f"{scheme}://{_direct_host}:{port}"
    else:
    ...
    if not silent:
        ...
        if local_https:
            ...
            print(f"  Local HTTPS: https://{local_host}:{port}/", flush = True)
...
def _build_arg_parser():
    ...
    parser.add_argument(
        "--ssl-certfile",
        default = None,
        help = "Path to a trusted local TLS certificate for native HTTPS serving.",
    )
    parser.add_argument(
        "--ssl-keyfile",
        default = None,
