...
def test_chat_settings_are_isolated_by_owner(tmp_path, monkeypatch):
    _reset_studio_db(tmp_path, monkeypatch)
    ...
    studio_db.upsert_chat_settings_merge(
        {"inferenceParams": {"temperature": 0.8}, "autoTitle": False},
        owner_username = "bob",
    )

    assert studio_db.list_chat_settings(owner_username = "alice") == {
        "inferenceParams": {"temperature": 0.2}
    }
    assert studio_db.list_chat_settings(owner_username = "bob") == {
        "autoTitle": False,
...
def test_prompts_and_prompt_lists_are_isolated_by_owner(tmp_path, monkeypatch):
    _reset_studio_db(tmp_path, monkeypatch)
    prompt_id = "shared-prompt-id"
    ...
    assert studio_db.list_prompt_entries(owner_username = "alice")[0]["text"] == "Only Alice"
    assert studio_db.list_prompt_entries(owner_username = "bob")[0]["text"] == "Only Bob"
    assert studio_db.list_prompt_lists_db(owner_username = "alice")[0]["name"] == "Alice list"
    assert studio_db.list_prompt_lists_db(owner_username = "bob")[0]["name"] == "Bob list"

    studio_db.delete_prompt_entry(prompt_id, owner_username = "alice")
    studio_db.delete_prompt_list_db(list_id, owner_username = "alice")

    assert studio_db.list_prompt_entries(owner_username = "alice") == []
    assert studio_db.list_prompt_lists_db(owner_username = "alice") == []
    assert studio_db.list_prompt_entries(owner_username = "bob")[0]["text"] == "Only Bob"
    assert studio_db.list_prompt_lists_db(owner_username = "bob")[0]["name"] == "Bob list"
