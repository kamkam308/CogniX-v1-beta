...
def test_filter_sensitive_data_redacts_nested_secret_keys():
    event = hmod.filter_sensitive_data(
        None,
        "info",
        {
            ...
            "extra": {
                ...
                "items": [{"api_key": "sk-unsloth-abcdefghijklmnopqrstuvwxyz"}],
            },
        },
    )

    assert event["password"] == "<redacted secret>"
    assert event["headers"]["Authorization"] == "<redacted secret>"
    assert event["extra"]["hf_token"] == "<redacted secret>"
    assert event["extra"]["items"][0]["api_key"] == "<redacted secret>"


def test_filter_sensitive_data_redacts_inline_token_patterns():
    event = hmod.filter_sensitive_data(
        None,
        "warning",
        {
            "message": (
                "download failed for hf_ABCDEFGHIJKLMNOPQRSTUVWXYZ "
                "with Authorization: Bearer abcdefghijklmnop and "
    ...
    assert "Bearer abcdefghijklmnop" not in message
    assert "sk-proj-abcdefghijklmnopqrstuvwxyz" not in message
