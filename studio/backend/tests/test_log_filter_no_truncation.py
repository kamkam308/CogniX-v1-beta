...
class TestCredentialRedaction:
    def test_hugging_face_token_is_redacted_in_strings(self):
        token = "hf_" + "A" * 36
        out = _run({"event": f"download failed for token={token}"})
        assert token not in out["event"]
        assert "<redacted secret>" in out["event"]

    def test_openai_and_cognix_keys_are_redacted_in_strings(self):
        openai_key = "sk-proj-" + "B" * 40
        cognix_key = "sk-unsloth-" + "c" * 32
        out = _run({"event": f"keys {openai_key} and {cognix_key}"})
        assert openai_key not in out["event"]
        assert cognix_key not in out["event"]
        assert out["event"].count("<redacted secret>") == 2

    def test_authorization_header_and_jwt_are_redacted(self):
        jwt = "eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiJrYW1pbCJ9.signature1234567890"
        out = _run({"event": f"Authorization: Bearer {jwt}"})
        assert jwt not in out["event"]
        assert out["event"] == "Authorization: Bearer <redacted secret>"

    def test_cookie_header_is_redacted(self):
        cookie = "cognix_refresh_token=refresh-secret-value"
        out = _run({"event": f"Cookie: {cookie}; Path=/api/auth"})
        assert cookie not in out["event"]
        assert "Cookie: <redacted secret>; Path=/api/auth" == out["event"]

    def test_sensitive_dict_keys_are_redacted_recursively(self):
        out = _run(
            {
                "event": "login",
        ...
        assert out["payload"]["hf_token"] == "<redacted secret>"
        assert out["payload"]["safe"] == "visible"

    def test_inline_password_assignment_is_redacted(self):
        out = _run({"event": "payload password='plain-secret' username=kamil"})
        assert "plain-secret" not in out["event"]
        assert out["event"] == "payload password='<redacted secret>' username=kamil"
