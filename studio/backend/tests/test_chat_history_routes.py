...
def _thread_payload(thread_id: str = "thread-1") -> chat_history.ChatThread:
    return chat_history.ChatThread(
        id = thread_id,
        title = "Test Chat",
        modelType = "base",
        modelId = "CogniX High",
...
def test_save_thread_rejects_existing_thread_owned_by_another_user(monkeypatch):
    called = False

    def fake_get_chat_thread(_id, **_kwargs):
        return {"id": _id, "ownerUsername": "alice"}
    ...
    monkeypatch.setattr(chat_history, "get_chat_thread", fake_get_chat_thread)
    monkeypatch.setattr(chat_history, "upsert_chat_thread", fake_upsert)
    monkeypatch.setattr(chat_history.auth_storage, "is_admin", lambda _subject: False)

    with pytest.raises(HTTPException) as exc:
        asyncio.run(chat_history.save_thread(_thread_payload(), current_subject = "bob"))

    assert exc.value.status_code == 404
    assert called is False


def test_admin_can_read_cross_owner_thread_messages(monkeypatch):
    monkeypatch.setattr(
        chat_history,
        "get_chat_thread",
        lambda _id, **_kwargs: {"id": _id, "ownerUsername": "alice"},
    )
    monkeypatch.setattr(chat_history.auth_storage, "is_admin", lambda _subject: True)
    monkeypatch.setattr(
        chat_history,
        "list_chat_messages",
        lambda thread_id: [_message("msg-1", thread_id).model_dump()],
    )

    response = asyncio.run(
        chat_history.get_thread_messages("thread-1", current_subject = "admin")
    )

    assert len(response.messages) == 1
    assert response.messages[0].threadId == "thread-1"


def test_admin_cannot_replace_cross_owner_thread_messages(monkeypatch):
    called = False
    ...
    monkeypatch.setattr(chat_history, "sync_chat_messages", fake_sync)
...
def test_admin_clear_history_only_targets_own_threads(monkeypatch):
    captured = {}

    def fake_clear_chat_history(**kwargs):
        captured.update(kwargs)

    monkeypatch.setattr(chat_history, "clear_chat_history", fake_clear_chat_history)

    response = asyncio.run(chat_history.clear_history(current_subject = "admin"))

    assert response == {"status": "deleted"}
    assert captured == {"owner_username": "admin", "include_all": False}
...
def test_fork_thread_404_when_source_missing(monkeypatch):
    monkeypatch.setattr(chat_history, "get_chat_thread", lambda _id, **_: None)
    with pytest.raises(HTTPException) as exc:
        asyncio.run(
...
def test_fork_thread_404_when_branch_message_missing(monkeypatch):
    monkeypatch.setattr(chat_history, "get_chat_thread", lambda _id, **_: {"id": _id, "title": "T"})
...
def test_fork_thread_happy_path(monkeypatch):
    ...
    monkeypatch.setattr(chat_history, "get_chat_thread", lambda _id, **_: source)
    monkeypatch.setattr(
        chat_history,
        "get_chat_message",
        lambda _t, _m: {
...
def test_admin_forks_cross_owner_thread_into_admin_account(monkeypatch):
    captured = {}
    source = {
        "id": "src",
        "title": "Original",
    ...
    def fake_fork_chat_thread(**kwargs):
        captured.update(kwargs)
        return {
            **source,
            "id": "new",
            ...
            "forkedFromThreadId": "src",
            "forkedFromMessageId": "m1",
            "ownerUsername": kwargs["owner_username"],
        }
    ...
    monkeypatch.setattr(chat_history, "fork_chat_thread", fake_fork_chat_thread)
    ...
    assert response.thread.ownerUsername == "admin"
...
def test_get_fork_count(monkeypatch):
    monkeypatch.setattr(chat_history, "get_chat_thread", lambda _id, **_: {"id": _id})
