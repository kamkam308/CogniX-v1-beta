...
def _load_route():
    ...
    try:
        ...
        auth_mod.get_current_jwt_subject = lambda: "test"
...
def test_status_handler_runs_off_event_loop(monkeypatch):
    ...
    monkeypatch.setattr(rl.auth_storage, "is_admin", lambda subject: subject == "admin")
    out = asyncio.run(rl.llama_update_status(force_refresh = False, current_subject = "admin"))
...
def test_update_handler_runs_off_event_loop(monkeypatch):
    ...
    monkeypatch.setattr(rl.auth_storage, "is_admin", lambda subject: subject == "admin")
    out = asyncio.run(rl.llama_update(current_subject = "admin"))
