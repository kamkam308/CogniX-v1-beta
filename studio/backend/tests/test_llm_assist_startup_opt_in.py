...
def test_settings_route_persists_helper_precache_toggle(monkeypatch):
    ...
    monkeypatch.delenv("UNSLOTH_HELPER_MODEL_DISABLE", raising = False)
    monkeypatch.setattr(settings_route.auth_storage, "is_admin", lambda subject: subject == "admin")
...
def test_settings_route_rejects_non_admin_helper_precache_toggle(monkeypatch):
    _install_fake_studio_db(monkeypatch)
    monkeypatch.setattr(settings_route.auth_storage, "is_admin", lambda subject: False)

    try:
        settings_route.update_helper_precache(
            settings_route.HelperPrecachePayload(enabled = True),
        ...
        raise AssertionError("non-admin helper pre-cache update should be rejected")
