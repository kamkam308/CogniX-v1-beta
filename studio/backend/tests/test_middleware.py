...
class TestSecurityHeadersMiddleware:
    ...
    def test_default_security_headers_present(self, main_module):
        ...
        assert r.headers["server"] == "cognix"

    def test_auth_responses_are_not_cached(self, main_module):
        app = FastAPI()
        app.add_middleware(main_module.SecurityHeadersMiddleware)
        ...
        r = TestClient(app).get("/api/auth/status")
        assert r.headers["cache-control"] == "no-store"
        assert r.headers["pragma"] == "no-cache"

    def test_hsts_is_sent_on_https_scope(self, main_module):
        ...
        asyncio.run(
            mw(
                {"type": "http", "scheme": "https", "path": "/plain", "headers": []},
                _receive,
                _send,
            )
        )
...
class TestSuspiciousRequestMiddleware:
    def test_rejects_obvious_sql_injection_probe_on_auth_route(self, main_module):
        app = FastAPI()
        app.add_middleware(main_module.SuspiciousRequestMiddleware)

        @app.get("/api/auth/status")
        async def status():
            return {"ok": True}

        r = TestClient(app).get("/api/auth/status?next=' OR 1=1 --")
        assert r.status_code == 400
        assert r.json()["detail"] == "Request rejected."

    def test_allows_normal_sensitive_route_query(self, main_module):
        app = FastAPI()
        app.add_middleware(main_module.SuspiciousRequestMiddleware)
        ...
        r = TestClient(app).get("/api/models/search?q=glm-5.2")
        assert r.status_code == 200
        assert r.json()["q"] == "glm-5.2"
...
def _make_cross_site_write_app(main_module, trusted_origins: tuple = ()):
    app = FastAPI()
    app.add_middleware(
        main_module.CrossSiteWriteMiddleware,
...
class TestCrossSiteWriteMiddleware:
    def test_same_origin_post_passes(self, main_module):
        app = _make_cross_site_write_app(main_module)
        c = TestClient(app)
        r = c.post(
            "/api/auth/refresh",
            headers = {
    ...
    def test_cross_origin_post_rejected_by_origin(self, main_module):
        app = _make_cross_site_write_app(main_module)
        c = TestClient(app)
        r = c.post(
            "/api/auth/refresh",
    ...
    def test_explicit_trusted_origin_passes(self, main_module):
        app = _make_cross_site_write_app(main_module, trusted_origins = ("http://localhost:5173",))
        c = TestClient(app)
        r = c.post(
            "/api/auth/refresh",
            headers = {
    ...
    def test_cross_site_fetch_metadata_rejected_without_origin(self, main_module):
        app = _make_cross_site_write_app(main_module)
        c = TestClient(app)
        r = c.post(
            "/v1/chat/completions",
    ...
    def test_cross_origin_referer_rejected_when_origin_missing(self, main_module):
        app = _make_cross_site_write_app(main_module)
        c = TestClient(app)
        r = c.post(
            "/api/auth/refresh",
    ...
    def test_missing_browser_headers_allows_local_api_clients(self, main_module):
        app = _make_cross_site_write_app(main_module)
        c = TestClient(app)
    ...
    def test_get_and_unprotected_writes_are_not_blocked(self, main_module):
        app = _make_cross_site_write_app(main_module)
        c = TestClient(app)
        assert c.get(
            "/api/auth/status",
...
class TestHealthAuthGate:
    ...
    def test_no_auth_exposes_launcher_bits(self, health_app):
        ...
        assert body["service"] == "CogniX Backend"
