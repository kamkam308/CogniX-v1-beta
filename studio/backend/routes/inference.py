...
from auth.authentication import get_current_subject, get_current_jwt_subject
...
async def _authenticate_header_or_query(request: Request, token: Optional[str]) -> str:
    ...
    return await get_current_jwt_subject(creds)
...
async def confirm_tool_call(
    request: ToolConfirmRequest, current_subject: str = Depends(get_current_jwt_subject)
):
...
async def get_api_monitor(current_subject: str = Depends(get_current_jwt_subject)):
...
async def get_api_monitor_entry(entry_id: str, current_subject: str = Depends(get_current_jwt_subject)):
...
async def get_status(current_subject: str = Depends(get_current_jwt_subject)):
    """
