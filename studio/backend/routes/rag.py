    ...
    with open(stored_path, "wb") as out:
        while True:
            ...
            if size > config.UPLOAD_MAX_BYTES:
                ...
                raise HTTPException(
                    status_code = 413,
                    detail = f"Uploaded file exceeds the {config.UPLOAD_MAX_BYTES} byte limit.",
                )
            out.write(block)
    if size == 0:
        os.remove(stored_path)
...
def _require_visible_thread(thread_id: str, subject: str) -> None:
    if get_chat_thread(
        thread_id,
...
def _require_visible_project(project_id: str, subject: str) -> None:
    if get_chat_project(
        project_id,
        owner_username = subject,
        include_all = _include_all_for_subject(subject),
...
def _require_visible_kb(conn, kb_id: str, subject: str) -> dict:
    kb = store.get_kb(
        conn,
        kb_id,
        owner_username = subject,
...
def _require_visible_document(conn, document_id: str, subject: str) -> dict:
    doc = store.get_document(conn, document_id)
    if doc is None:
        raise HTTPException(status_code = 404, detail = "Document not found")
    if doc.get("kb_id"):
        _require_visible_kb(conn, doc["kb_id"], subject)
    if doc.get("thread_id"):
        _require_visible_thread(doc["thread_id"], subject)
    if doc.get("project_id"):
        _require_visible_project(doc["project_id"], subject)
    return doc


def _require_visible_job(job_id: str, subject: str) -> dict:
    row = ingestion.get_job_status(job_id)
    if row is None:
        raise HTTPException(status_code = 404, detail = "Job not found")
    conn = rag_db.get_connection()
    try:
        _require_visible_document(conn, row["document_id"], subject)
    finally:
        conn.close()
    return row
...
def list_knowledge_bases(subject: str = Depends(get_current_jwt_subject)) -> dict:
    _require_rag()
    conn = rag_db.get_connection()
    try:
        kbs = store.list_kbs(
...
def create_knowledge_base(
    payload: CreateKbRequest, subject: str = Depends(get_current_jwt_subject)
) -> dict:
    _require_rag()
    conn = rag_db.get_connection()
    try:
...
def update_knowledge_base(
    ...
    subject: str = Depends(get_current_jwt_subject),
) -> dict:
    _require_rag()
    conn = rag_db.get_connection()
    try:
        _require_visible_kb(conn, kb_id, subject)
        sets, params = [], []
...
def delete_knowledge_base(kb_id: str, subject: str = Depends(get_current_jwt_subject)) -> dict:
    _require_rag()
    conn = rag_db.get_connection()
    try:
        _require_visible_kb(conn, kb_id, subject)
...
def list_kb_documents(kb_id: str, subject: str = Depends(get_current_jwt_subject)) -> dict:
    _require_rag()
    conn = rag_db.get_connection()
    try:
        _require_visible_kb(conn, kb_id, subject)
...
async def upload_thread_document(
    ...
    _require_rag()
    _require_visible_thread(thread_id, subject)
    stored_path, filename = _save_upload(file)
    document_id, job_id = ingestion.start_ingestion(
...
def list_thread_documents(thread_id: str, subject: str = Depends(get_current_jwt_subject)) -> dict:
    _require_rag()
    _require_visible_thread(thread_id, subject)
    conn = rag_db.get_connection()
    try:
...
async def upload_project_document(
    ...
    _require_rag()
    _require_visible_project(project_id, subject)
    stored_path, filename = _save_upload(file)
    document_id, job_id = ingestion.start_ingestion(
...
def list_project_documents(project_id: str, subject: str = Depends(get_current_jwt_subject)) -> dict:
    _require_rag()
    _require_visible_project(project_id, subject)
    conn = rag_db.get_connection()
    try:
...
def delete_document(document_id: str, subject: str = Depends(get_current_jwt_subject)) -> dict:
    _require_rag()
    conn = rag_db.get_connection()
    try:
        doc = _require_visible_document(conn, document_id, subject)
...
def job_status(job_id: str, subject: str = Depends(get_current_jwt_subject)) -> dict:
    _require_rag()
    row = _require_visible_job(job_id, subject)
    return {
        "id": row["id"],
...
def job_events(job_id: str, subject: str = Depends(get_current_jwt_subject)) -> StreamingResponse:
    _require_rag()
    _require_visible_job(job_id, subject)
...
@router.post("/search")
def search(payload: SearchRequest, subject: str = Depends(get_current_jwt_subject)) -> dict:
    _require_rag()
    conn = rag_db.get_connection()
    try:
        if payload.kb_id:
            _require_visible_kb(conn, payload.kb_id, subject)
            scope = store.kb_scope(payload.kb_id)
        else:
            scopes = []
            if payload.project_id:
                _require_visible_project(payload.project_id, subject)
                scopes.append(store.project_scope(payload.project_id))
            if payload.thread_id:
                _require_visible_thread(payload.thread_id, subject)
...
def preview_target(
    ...
    try:
        ...
        if doc is None:
            raise HTTPException(status_code = 404, detail = "Document not found")
        _require_visible_document(conn, document_id, subject)
...
def document_file_url(document_id: str, subject: str = Depends(get_current_jwt_subject)) -> dict:
    """Mint a short-lived signed URL for the source file."""
    _require_rag()
    conn = rag_db.get_connection()
    try:
        doc = _require_visible_document(conn, document_id, subject)
        if not doc.get("stored_path"):
...
def document_file_signed(document_id: str, token: str = Query(...)) -> FileResponse:
    ...
    uploads = os.path.realpath(str(rag_uploads_root()))
    if os.path.commonpath([uploads, os.path.realpath(stored_path)]) != uploads:
