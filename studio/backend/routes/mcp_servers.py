...
async def list_mcp_servers(current_subject: str = Depends(get_current_jwt_subject)):
...
async def create_mcp_server(
    payload: McpServerCreate, current_subject: str = Depends(get_current_jwt_subject)
):
    _require_admin(current_subject)
...
async def delete_mcp_server(server_id: str, current_subject: str = Depends(get_current_jwt_subject)):
    _require_admin(current_subject)
    old = mcp_servers_db.get_server(server_id)
    if not old:
        raise HTTPException(status_code = 404, detail = "MCP server not found")
...
async def refresh_mcp_server_tools(
    server_id: str, current_subject: str = Depends(get_current_jwt_subject)
):
    _require_admin(current_subject)
...
async def import_mcp_servers(
    payload: McpServerImportRequest, current_subject: str = Depends(get_current_jwt_subject)
):
...
async def test_mcp_server(
    payload: McpServerTestRequest, current_subject: str = Depends(get_current_jwt_subject)
):
    _require_admin(current_subject)
