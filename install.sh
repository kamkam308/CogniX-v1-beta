...
# CogniX Installer
#
# Usage:  curl -fsSL https://raw.githubusercontent.com/kamkam308/CogniX-v1-beta/main/install.sh | sh
#         wget  -qO- https://raw.githubusercontent.com/kamkam308/CogniX-v1-beta/main/install.sh | sh
...
#   curl -fsSL https://raw.githubusercontent.com/kamkam308/CogniX-v1-beta/main/install.sh | UNSLOTH_NO_TORCH=1 sh    # skip PyTorch (GGUF-only)
#   curl -fsSL https://raw.githubusercontent.com/kamkam308/CogniX-v1-beta/main/install.sh | UNSLOTH_PYTHON=3.12 sh   # pin Python version
#   curl -fsSL https://raw.githubusercontent.com/kamkam308/CogniX-v1-beta/main/install.sh | UNSLOTH_STUDIO_HOME=/abs/path sh
...
_smart_apt_install() {
    ...
    if command -v sudo >/dev/null 2>&1; then
        ...
        case "$REPLY" in
            [nN]*)
                echo ""
                echo "    Please install these packages first, then re-run CogniX setup:"
        ...
        echo "    Please install these packages as root, then re-run CogniX setup:"
...
# Usage: create_studio_shortcuts <runtime_exe> <os>
# Creates the shared CogniX launcher script,
...
create_studio_shortcuts() {
    ...
    # Validate exe
    if [ ! -x "$_css_exe" ]; then
        echo "[WARN] Cannot create shortcuts: runtime executable not found at $_css_exe"
        return 0
    fi
    ...
    _css_icon_png="$_css_data_dir/cognix.png"
    _css_gem_png="$_css_data_dir/cognix-app-icon.png"
...
if [ -t 1 ]; then
    ...
    echo "CogniX did not become healthy within ${TIMEOUT_SEC}s." >&2
...
LAUNCHER_EOF
    ...
    # cognix-logo-512.png is used for both Linux and macOS icons.
    _css_script_dir=""
    if [ -n "${0:-}" ] && [ -f "$0" ]; then
        _css_script_dir=$(cd "$(dirname "$0")" 2>/dev/null && pwd) || true
    fi

    # Try to find cognix-logo-512.png from installed package (site-packages) or local repo.
    _css_found_icon=""
    ...
    for _sp in "$_css_venv_dir"/lib/python*/site-packages/unsloth/studio/frontend/public; do
        if [ -f "$_sp/cognix-logo-512.png" ]; then
            _css_found_icon="$_sp/cognix-logo-512.png"
        fi
    done
    # Check local repo (when running from clone)
    if [ -z "$_css_found_icon" ] && [ -n "$_css_script_dir" ] && [ -f "$_css_script_dir/studio/frontend/public/cognix-logo-512.png" ]; then
        _css_found_icon="$_css_script_dir/studio/frontend/public/cognix-logo-512.png"
    fi

    # Copy or download the CogniX logo (used for both Linux icon and macOS icns).
    if [ -n "$_css_found_icon" ]; then
        cp "$_css_found_icon" "$_css_icon_png" 2>/dev/null || true
        cp "$_css_found_icon" "$_css_gem_png" 2>/dev/null || true
    else
        download "https://raw.githubusercontent.com/kamkam308/CogniX-v1-beta/main/studio/frontend/public/cognix-logo-512.png" "$_css_icon_png" 2>/dev/null || true
...
DESKTOP_EOF
        ...
        # ── macOS: .app bundle ──
        _css_app="$HOME/Applications/CogniX.app"
...
STUB_EOF
        ...
        # Build AppIcon.icns from the CogniX app icon.
        ...
        # Symlink on Desktop
        if [ -d "$HOME/Desktop" ]; then
            ln -sf "$_css_app" "$HOME/Desktop/CogniX" 2>/dev/null || true
        fi
        _css_created=1
        ...
        # install's "CogniX.lnk" in the same folder. Per-distro suffix.
        if [ -n "$_css_distro" ]; then
            _css_lnk_name="CogniX (WSL - ${_css_distro}).lnk"
        else
            _css_lnk_name="CogniX (WSL).lnk"
        fi
...
# Best-effort: fetch the CogniX icon to a stable Windows path (shared with a
# native install if one exists) so the WSL shortcut shows the proper icon.
\$iconDir = Join-Path \$env:LOCALAPPDATA 'CogniX'
\$iconPath = Join-Path \$iconDir 'cognix.ico'
\$preIconHash = \$null
...
if (-not (Test-Path -LiteralPath \$iconPath)) {
    try {
        ...
        Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/kamkam308/CogniX-v1-beta/main/studio/src-tauri/icons/icon.ico' -OutFile \$iconPath -UseBasicParsing -ErrorAction Stop
    } catch {}
}
\$hasIcon = \$false
if (Test-Path -LiteralPath \$iconPath) {
...
foreach (\$dir in \$locations) {
    ...
    \$shortcut.Description = 'Launch CogniX (WSL)'
...
_maybe_reroute_strixhalo_to_2404() {
    ...
    if [ -n "${UNSLOTH_WSL_REROUTE_CMD:-}" ]; then
        ...
        _rr_cmd="curl -fsSL https://raw.githubusercontent.com/kamkam308/CogniX-v1-beta/main/install.sh | sh -s --$_rr_args"
    else
        _rr_cmd="curl -fsSL https://raw.githubusercontent.com/kamkam308/CogniX-v1-beta/main/install.sh | sh"
    fi
    ...
    substep "  wsl -d $_rr_target -- bash -lc 'curl -fsSL https://raw.githubusercontent.com/kamkam308/CogniX-v1-beta/main/install.sh | sh'"
    substep "Continuing CPU-only in Ubuntu ${_rr_ver:-this distro} for now." "$C_WARN"
...
if [ -x "$VENV_DIR/bin/python" ]; then
    ...
    if [ "$_STUDIO_HOME_REDIRECT" = "env" ] \
       ...
       && [ ! -f "$STUDIO_HOME/bin/unsloth" ]; then
        echo "ERROR: $VENV_DIR already exists but does not look like a CogniX install." >&2
    ...
    */cpu)
        if [ "$SKIP_TORCH" = false ] && [ "$OS" != "macos" ]; then
            ...
            if [ "$OS" = "wsl" ]; then
                ...
                substep "AMD ROCm users: see the CogniX GPU setup notes in the project repository."
            fi
            substep "Re-run with --no-torch for GGUF-only (faster, no PyTorch):"
            substep "  curl -fsSL https://raw.githubusercontent.com/kamkam308/CogniX-v1-beta/main/install.sh | sh -s -- --no-torch"
        fi
        ;;
    */rocm*|*/gfx*)
...
if [ "$_MIGRATED" = true ]; then
    ...
    tauri_log "STEP" "Installing CogniX"
        ...
        if [ "$_STUDIO_HOME_REDIRECT" = "env" ]; then
            ...
            if [ -n "$_SHELL_PROFILE" ]; then
                if ! grep -q '\.local/bin' "$_SHELL_PROFILE" 2>/dev/null; then
                    echo '' >> "$_SHELL_PROFILE"
                    echo '# Added by CogniX installer' >> "$_SHELL_PROFILE"
    ...
    case "${_reply:-y}" in
        [Yy]*|"")
            ...
            if [ "$_LAUNCH_EXIT" -ne 0 ] && [ "$_MIGRATED" = true ]; then
                echo ""
                echo "CogniX failed to start after migration."
                echo "   Your migrated environment may be incompatible."
                echo "   To fix, remove the environment and reinstall:"
                echo ""
                echo "   rm -rf $VENV_DIR"
                echo "   curl -fsSL https://raw.githubusercontent.com/kamkam308/CogniX-v1-beta/main/install.sh | sh"
                echo ""
            fi
            exit "$_LAUNCH_EXIT"
            ;;
